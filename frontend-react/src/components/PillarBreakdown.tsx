...
export const PillarBreakdown: React.FC<PillarBreakdownProps> = ({ pattern }) => {
  ...
  // Extract raw pillar scores (0 - 1.0 or 0 - 100)
  ...
  const pillars = [
    {
      ...
      description: "Density of incidents mapped tightly within spatial proximity radius.",
    },
    {
      id: "temporal",
    ...
    },
    {
      ...
      description: "Recurrence frequency and rate of incoming signal reports in this hotspot.",
    },
    {
      id: "trend",
      ...
      description: "Velocity change comparing current 24h window against baseline baseline history.",
    },
    {
      id: "diversity",
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-xs font-mono font-semibold uppercase tracking-wider text-slate-400">
          Composite 6-Pillar Risk Engine Breakdown
        </h4>
        <span className="text-[11px] font-mono text-slate-500">Weights sum to 100%</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {pillars.map((pillar) => {
          ...
          return (
            <div
              key={pillar.id}
              className="p-3.5 rounded-lg bg-slate-900/60 border border-white/5 hover:border-white/10 transition-colors flex flex-col justify-between gap-2"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  <div className={`p-1.5 rounded-md bg-white/5 ${pillar.textColor}`}>
                    <Icon className="h-4 w-4" />
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-medium text-slate-200">{pillar.name}</span>
                      <span className="text-[10px] font-mono px-1 py-0.2 rounded bg-slate-800 text-slate-400">
                        {pillar.weight}
                      </span>
                    </div>
                    <span className="text-[11px] text-slate-400">{pillar.detail}</span>
                  </div>
                </div>
              ...
              <div className="space-y-1">
                <Progress value={pillar.score} indicatorClassName={pillar.color} className="h-1.5" />
